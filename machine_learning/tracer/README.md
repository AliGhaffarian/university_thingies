...
- `loader_and_logger.c`: userspace loader + ring buffer consumer. Loads BPF object, attaches, polls ring buffer and writes `PID:syscall:comm` lines to stdout and `trace.log`. Also writes program name into `progs` BPF map to indicate which processes to trace.
